...
resource "aws_cloudwatch_log_group" "lambda_collect_daily_reporter" {
  name              = "/aws/lambda/${local.name_prefix}-collect-daily-reporter"
  retention_in_days = var.cloudwatch_log_retention_days

  tags = {
    Name = "${local.name_prefix}-collect-daily-reporter-logs"
  }
}
